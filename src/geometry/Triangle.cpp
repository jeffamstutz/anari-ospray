...
Triangle::Triangle(OSPRayGlobalState *s)
    : Geometry(s, "mesh"),
      m_index(this),
...
void Triangle::finalize()
{
  ...
  if (m_index) {
    ...
    auto id = ospNewSharedData(indices.data(), OSP_VEC3UI, indices.size() / 3);
    ospSetParam(og, "index", OSP_DATA, &id);
    ospRelease(id);
  }
  ospCommit(og);
...
void Triangle::setColorAttribute(Attribute attr, OSPGeometricModel om)
{
...
void Triangle::setTextureCoordinateAttribute(Attribute attr)
{
  auto og = osprayGeometry();
